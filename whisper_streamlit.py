# -*- coding: utf-8 -*-
"""whisper_streamlit.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sus541UcUO4b9RZZEPSlSZTd1PW5ZgUa
"""
...
# ✅ Custom CSS for Modern Black-Gold Theme
st.markdown("""
    <style>
...
# ✅ Load Whisper Model (Using Small Model Only)
@st.cache_resource
def load_model():
...
# ✅ Load NLP Models (Optimized)
@st.cache_resource
def load_nlp_models():
    sentiment_model = pipeline("sentiment-analysis", model="distilbert-base-uncased-finetuned-sst-2-english")
    emotion_model = pipeline("text-classification", model="j-hartmann/emotion-english-distilroberta-base")
    summarizer = pipeline("summarization", model="t5-small")  # Lighter model to prevent memory issues
...
def preprocess_audio(audio_path):
    """Ensures the audio is in the correct format before transcription."""
    try:
...
def summarize_text(text):
    """Generates a high-quality summary using a lightweight model."""
...
def text_to_speech(text, lang="en"):
    """Converts transcribed text back into speech."""
    try:
...
if uploaded_file:
    ...
    # ✅ Preprocess Audio (Convert to mono and 16kHz)
    ...
    if st.session_state.transcribed_text:
        ...
        st.text_area("", st.session_state.transcribed_text, height=120)

        st.markdown("💬 **Sentiment Analysis:**")
        st.write(st.session_state.sentiment_result)

        st.markdown("🎭 **Emotion Analysis:**")
        st.write(st.session_state.emotion_result)

        st.markdown("📄 **Summarized Text:**")
        st.text_area("", st.session_state.summary_result, height=80)

        if translation_mapping[translation_lang]:
            st.markdown("🌍 **Translated Text:**")
            st.text_area("", st.session_state.translated_text, height=80)
